...
class minHeap:
    ...
    def insertNew(self, element, weight, cost, path):
        if self.size >= self.maxsize:
            return
        insertitem = [element, weight, cost, path]
...
class priorityQueueHeap:
    ...
    def insertNew(self, element, weight, cost, path):
        self.heap.insertNew(element, weight, cost, path)
        self.size += 1
...
task_average_time = mean(timelist)
...
print('Average time taken: ', task_average_time)
...
def ucs2(g, src, destination):
    ...
    while not queue.isEmpty():
        ...
        visited[current] = 1
        if current == destination:
            return node[1], node[2], node[3], visited

        for neighbour, info in g[current].items():
            edge_distance = info[0]
            edge_cost = info[1]
            if node[2] + edge_cost > 287932:
                continue
            if distance[neighbour] > node[1] + edge_distance or \
                accumulated_cost[neighbour] > node[2] + edge_cost:
                
                distance[neighbour] = node[1] + edge_distance
                accumulated_cost[neighbour] = node[2] + edge_cost

                queue.insertNew(neighbour, distance[neighbour], accumulated_cost[neighbour], [*node[3], neighbour])

timelist = []
for i in tqdm(range(1,21)):
    start = time.time()
    g = graph.getGraph()
    distance, accumulated_cost, path, visited = ucs2(g, 1, 50)
    end = time.time()
    timelist.append(end - start)

task_average_time = mean(timelist)
print('Shortest path: ', '->'.join([str(elem) for elem in path]))
print('Shortest distance: ', distance)
print('Total energy cost: ', accumulated_cost)
print('Average time taken: ', task_average_time)
print('Number of nodes visited: ', sum(visited.values()))
print()
...
def astar(g, src, destination):
    ...
    for i in range(1, len(g) + 1):
        ...
        accumulated_cost[i] = 0
    distance[src] = 0
    queue.insertNew(src, heuristics[1], (0, 0), [1])
    ...
    while not queue.isEmpty():
        ...
        visited[current] = 1
        if current == destination:
            return node[2][0], node[2][1], node[3], visited

        for neighbour, info in g[current].items():
            edge_distance = info[0]
            edge_cost = info[1]
            if node[2][1] + edge_cost > 287932:
                continue
            if distance[neighbour] > node[2][0] + edge_distance or \
                accumulated_cost[neighbour] > node[2][1] + edge_cost:
                
                distance[neighbour] = node[2][0] + edge_distance
                accumulated_cost[neighbour] = node[2][1] + edge_cost
                queue.insertNew(neighbour, distance[neighbour] + heuristics[neighbour],
                    (distance[neighbour], accumulated_cost[neighbour]), [*node[3], neighbour])

timelist = []
for i in tqdm(range(1,21)):
    start = time.time()
    g = graph.getGraph()
    distance, accumulated_cost, path, visited = astar(g, 1, 50)
    end = time.time()
    timelist.append(end - start)
